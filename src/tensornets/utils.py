...
def get_all(scope=None, tensor_names=None):

    # NOTE: Uncomment to pickle tensor names
    # with open('tensor_names.pickle', 'wb') as f:
        # pickle.dump(tensor_names, f)

    scope = parse_scopes(scope)[0]
    return tf.get_collection(__allofem__, scope=scope +'/')

def get_middles(scope=None):
    scope = parse_scopes(scope)[0]
    relu = tf.get_collection('1/relu', scope=scope+'/')
            ...
            with tf.variable_scope(scope, reuse=reuse):
                ...
                if func.__name__ == 'wrapper':
                    ...
                    for tensor in _outs:
                        ...
                        allofem = collect_named_outputs(__allofem__, _scope, tensor)
                    
                    # Adds middles to all the outputs
                    for i in p0(name)[0]:
                        test = collect_named_outputs(__middles__, _scope, _outs[i])
